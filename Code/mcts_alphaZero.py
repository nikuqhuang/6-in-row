...
class TreeNode(object):
    ...
    def update_recursive(self, leaf_value, hand):
        ...
        if self._parent:
            self._parent.update_recursive(-value, hand-1)
...
class MCTS(object):
    ...
    def _playout(self, state):
        ...
        if not end:
            if state.is_defand2():
                node.expand_auto(state.a)
                node.expand_auto(state.b)
            elif state.is_defand1():
                node.expand_auto(state.a)
            else:
        ...
        node.update_recursive(-leaf_value, state.hand-1)
